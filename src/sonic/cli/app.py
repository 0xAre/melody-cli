...
# Shortcut: `sonic <url>` → langsung download tanpa perlu subcommand
...
def cmd_get(
    ...
    """Shortcut: sonic get <url>  =  sonic download url <url>"""
    ...
    out_dir = resolve_output_dir(output or None, Path(cfg.get("output_dir", "~/Music")).expanduser())
    is_playlist = "playlist" in url or "list=" in url

    if is_playlist:
        s = download_playlist(url, out_dir, q)
        console.print(f"\n[green]Berhasil: {s.ok}[/green]  [yellow]Skip: {s.skipped}[/yellow]  [red]Gagal: {s.failed}[/red]")
    else:
...
def main_callback(
    version: bool = typer.Option(
        ...
        is_eager=True, help="Tampilkan versi"
    ),
) -> None:
    pass
